...
stan_model=stan_model(file='section 3-1 simulation/model.stan')
...
write("model, y, var, time, sample, n.obs, N, mean, icl, icu",file='section 3-1 simulation/simul_data_multinomial.csv')
write("model, n.obs, sample, N, time_spent",file='section 3-1 simulation/time_data_multinomia.csv')

#### If continuing a simulation ####
# 
# init=449 # The last completed set in the last run
# 
# read.csv('section 3-1 simulation/simul_data_multinomial.csv') %>%
#   filter(sample<init) %>%
#   write.csv(file='section 3-1 simulation/simul_data_multinomial.csv',row.names=FALSE)
# read.csv('section 3-1 simulation/time_data_multinomial.csv') %>%
#   filter(sample<init) %>%
#   write.csv(file='section 3-1 simulation/time_data_multinomial.csv',row.names=FALSE)
####################################
...
for(rep in init:2000){
  for(N.smp in c(1,2,5,10,20,50,100)){
    ...
    for(t in c(2,5,10,20,50)){
      ...
      write.table(simul.data,file='section 3-1 simulation/simul_data_multinomial.csv',append =TRUE,col.names = FALSE,row.names = FALSE,sep=',',dec='.')
      ...
      write.table(time.data,file='section 3-1 simulation/time_data_multinomial.csv',append =TRUE,col.names = FALSE,row.names = FALSE,sep=',',dec='.')
    }
  }
}