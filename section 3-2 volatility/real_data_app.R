...
if(FALSE){
  stan_fit=stan(
    file='section 3-2 volatility/volat_model_sim.stan',
    data=list(n=T,y=data_ori,W=W1,m0=m0,C0=C0)
  )
  
  stan_smp=rstan::extract(stan_fit)
  save(stan_smp,file='section 3-2 volatility/stan_fit_ibm.bkp')
}else{
  load(file='section 3-2 volatility/stan_fit_ibm.bkp')
}
